...
local ftgroup = vim.api.nvim_create_augroup("FileTypesGroup", { clear = true })
...
-- vim.api.nvim_create_autocmd("", {command = "BufNewFile,BufEnter,BufRead *.pic.yml call DetectSls()"}, group=ftgroup)
...
local pythonsettings = vim.api.nvim_create_augroup("pythonsettings", { clear = true })
vim.api.nvim_create_autocmd("Filetype python", { command = 'echo "python!"' })

-- disable both hard wrapping and soft wrapping
vim.api.nvim_create_autocmd("Filetype python", { command = "NoWrap" })

-- also show colorcolumn at 88 characters
vim.api.nvim_create_autocmd("Filetype python", { command = "setlocal colorcolumn=88" })

-- enable autoindent on new line
vim.api.nvim_create_autocmd("FileType python", { command = "setlocal autoindent" })

-- set tabs to have a width of 4 spaces
vim.api.nvim_create_autocmd("FileType python", { command = "setlocal tabstop=4" })

-- set tabs to have a maximum width of 4 spaces
vim.api.nvim_create_autocmd("FileType python", { command = "setlocal softtabstop=4" })

-- set the shift operators (`<<` and `>>`) to insert 4 spaces
vim.api.nvim_create_autocmd("FileType python", { command = "setlocal shiftwidth=4" })

local texmdgroup = vim.api.nvim_create_augroup("latexmarkdownsettings", { clear = true })

local texsettings = vim.api.nvim_create_augroup("texsettings", { clear = true })
-- enable hard wrapping (insert enter) at 88 characters [tex, md, vimwiki]
vim.api.nvim_create_autocmd("FileType tex,markdown,vimwiki", { command = "HardWrap", group = ftgroup })
